...
def create_new_task(
    ...
    """
    try:
        task.status = status.value if status else None
...
async def edit_task_endpoint(
    ...
    """
    try:
        updated_task.status = status  # Assign the status obtained from the Query
...
@router.put("/update/{task_id}", response_model=UpdateTask,summary="Update the task status")
async def update_task_status(
    task_id: int,
    status: TaskStatus,  # Correct way to use enum as a type
    task: CreateHistory,
    db: Session = Depends(get_db),
    current_user: get_current_user = Depends(),
):
    ...
    """
    try:
        return update_task(db, task_id, CreateHistory(status=status.value, comments=task.comments), current_user)
    except ValueError:
